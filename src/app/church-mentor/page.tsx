...
export default function ChurchMentorPage() {
  ...
  const [suggestions, setSuggestions] = useState<ChurchAutoSearchResult[]>([]);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [recentSearches, setRecentSearches] = useState<string[]>([]);
  const [popularChurches, setPopularChurches] = useState<PopularChurch[]>([]);
  const [popularBoards, setPopularBoards] = useState<ChurchBoard[]>([]);
  ...
  // 최근 검색 기록 로드
  useEffect(() => {
    const saved = localStorage.getItem('recentChurchSearches');
    if (saved) {
  ...
  const addToRecentSearches = (term: string) => {
    ...
    localStorage.setItem('recentChurchSearches', JSON.stringify(updated));
  };
  ...
  const fetchPopularChurches = async () => {
    try {
      ...
      if (data.status === 200 && data.data) {
        setPopularChurches(data.data);
      }
    } catch (error) {
      console.error('인기 교회 로딩 오류:', error);
    }
  };
  ...
  // 자동완성 선택
  const handleSuggestionClick = (suggestion: ChurchAutoSearchResult) => {
    setSearchTerm(suggestion.churchName);
    setShowSuggestions(false);
    setError(null);
    addToRecentSearches(suggestion.churchName);
    // 자동완성 선택 시에는 바로 상세 페이지로 이동
    router.push(`/church-mentor/${encodeURIComponent(suggestion.churchName)}`);
  };
  ...
  const performSearchAndNavigate = async (searchTerm: string) => {
    try {
      ...
      // 디버깅을 위한 로그 출력
      console.log('교회 자동완성 API 결과:', autoData);
      ...
      if (Array.isArray(autoData)) {
        if (autoData.length === 1) {
          ...
          router.push(`/church-mentor/${encodeURIComponent(autoData[0].churchName)}`);
        } else {
          // 2개 이상의 결과가 있으면 검색 결과 페이지로 이동
          router.push(`/church-search?name=${encodeURIComponent(searchTerm)}`);
        }
      } else {
  ...
  const handleRecentSearchClick = (term: string) => {
    router.push(`/church-search?name=${encodeURIComponent(term)}`);
  };

  // 인기 교회 클릭
  const handlePopularChurchClick = (church: PopularChurch) => {
    router.push(`/church-mentor/${encodeURIComponent(church.churchName)}`);
  };

  // 인기 후기 클릭
  const handlePopularBoardClick = (board: ChurchBoard) => {
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    if (!requestForm.churchName.trim() || !requestForm.churchPastor.trim() || !requestForm.churchLocation.trim()) {
      alert('모든 필드를 입력해주세요.');
      return;
    }
    ...
    try {
      ...
      console.error('교회 추가 요청 오류:', error);
      alert('교회 추가 요청 전송 중 오류가 발생했습니다. 다시 시도해주세요.');
    } finally {
  ...
  return (
    <div className="relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center px-4 sm:px-6 lg:px-8">
          <div className="text-center text-white w-full">
            <h1 className="font-black-han-sans text-2xl sm:text-3xl md:text-4xl lg:text-5xl mb-6 sm:mb-8 text-shadow-lg leading-tight">
              <span className="block">세상 모든 교회 정보,</span>
              <span className="block">교회 오빠가 알려줄게</span>
            </h1>
            ...
            <form onSubmit={handleSearch} className="max-w-xl mx-auto">
              <div className="relative" ref={searchRef}>
                <div className="flex items-center bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 overflow-hidden mx-2 sm:mx-0">
                  ...
                  <button
                    type="submit"
                    className="px-3 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-semibold text-sm sm:text-base transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-2 flex items-center space-x-1 sm:space-x-2 whitespace-nowrap min-w-[70px] sm:min-w-[80px]"
                  >
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1">
                            <div className="font-semibold text-gray-900">{suggestion.churchName}</div>
                            <div className="text-sm text-gray-500">📍 {suggestion.churchAddr}</div>
                            <div className="text-xs text-gray-400">👨‍💼 {suggestion.churchPastor}</div>
                          </div>
                          <div className="text-red-400 opacity-0 group-hover:opacity-100 transition-all duration-200">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="mt-6">
                <div className="flex items-center gap-3 flex-wrap justify-center">
                  ...
                  <button
                    onClick={() => {
                      setRecentSearches([]);
                      localStorage.removeItem('recentChurchSearches');
                    }}
                  ...
                  {recentSearches.map((search, index) => (
                    ...
                    >
                      <span className="text-red-700 font-medium text-sm">{search}</span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
      ...
      {/* 교회 추가 요청 CTA 배너 - 검색창 바로 아래 */}
      <div className="bg-red-500 py-6 shadow-lg">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-red-50 text-sm md:text-base">
                지금 요청하고 더 많은 교회 정보를 공유해보세요
              </p>
            </div>
            <button
              type="button"
              onClick={() => setShowRequestModal(true)}
              className="flex items-center space-x-2 px-6 py-3 bg-white text-red-600 font-semibold rounded-lg shadow-lg hover:bg-red-50 transition-all duration-300 hover:scale-105 hover:shadow-xl whitespace-nowrap"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                <div className="p-3 border-b border-gray-200">
                  <div className="flex items-center justify-between">
                    <h2 className="text-base font-bold text-gray-900">인기 교회 TOP 10</h2>
                    <button
                ...
                <div className="p-3">
                  <div className="grid grid-cols-1 gap-1.5">
                    {popularChurches.map((church, index) => (
                      <div
                        key={church.churchIdx}
                        onClick={() => handlePopularChurchClick(church)}
                        className={`group p-1.5 rounded-lg border border-gray-200 hover:border-red-300 hover:shadow-md transition-all duration-300 cursor-pointer transform hover:scale-105 ${
                          isRefreshing ? 'animate-pulse' : ''
                        }`}
                      >
                        <div className="flex items-center space-x-1.5">
                          <div className={`w-4 h-4 rounded-full flex items-center justify-center text-white font-bold text-xs ${
                            index < 3 ? 'bg-gradient-to-r from-red-400 to-pink-500' : 'bg-gradient-to-r from-gray-400 to-gray-600'
                          }`}>
                            {index + 1}
                          </div>
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-gray-900 group-hover:text-red-600 transition-colors duration-200 text-xs truncate">
                              {church.churchName}
                            </h3>
                            <p className="text-xs text-gray-500 truncate">📍 {church.churchLocation}</p>
                            <p className="text-xs text-gray-400 truncate">{church.churchType}</p>
                          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-gray-900">교회 추가 요청</h2>
                <button
              ...
              <p className="text-gray-600 mt-2">새로운 교회 정보를 요청해주세요.</p>
            </div>
            ...
            <form onSubmit={handleRequestSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  id="churchName"
                  value={requestForm.churchName}
                  onChange={(e) => setRequestForm({...requestForm, churchName: e.target.value})}
              ...
              {/* 교회 목사 */}
              <div>
                <label htmlFor="churchPastor" className="block text-sm font-medium text-gray-700 mb-2">
                  교회 목사 *
                </label>
                <input
                  type="text"
                  id="churchPastor"
                  value={requestForm.churchPastor}
                  onChange={(e) => setRequestForm({...requestForm, churchPastor: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
              ...
              {/* 교회 종류 */}
              <div>
                <label htmlFor="churchType" className="block text-sm font-medium text-gray-700 mb-2">
                  교회 종류 *
                </label>
                <select
                  id="churchType"
                  value={requestForm.churchType}
                  onChange={(e) => setRequestForm({...requestForm, churchType: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  required
                >
              ...
              {/* 교회 위치 */}
              <div>
                <label htmlFor="churchLocation" className="block text-sm font-medium text-gray-900 mb-2">
                  교회 위치 *
                </label>
                <input
                  type="text"
                  id="churchLocation"
                  value={requestForm.churchLocation}
                  onChange={(e) => setRequestForm({...requestForm, churchLocation: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
