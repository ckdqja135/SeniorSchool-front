...
function CompanySearchContent() {
  ...
  const [searchResults, setSearchResults] = useState<CompanyAutoSearchResult[]>([]);
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      ...
      try {
        ...
        const response = await fetch(`${backendURL}/search/comp/?compName=${encodeURIComponent(searchName)}`);
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span className="font-medium">회사 오빠</span>
              </Link>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {searchResults.map((company, index) => (
                <div
                  ...
                  className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 cursor-pointer hover:shadow-md transition-all duration-200 hover:border-purple-300"
                >
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-semibold text-gray-900 truncate">{company.compName}</h3>
                      <p className="text-sm text-gray-500 mt-1">📍 {company.compLocation}</p>
                      <div className="mt-2 space-y-1">
                        <div className="flex justify-between text-xs text-gray-400">
                          <span>회사 유형:</span>
                          <span className="font-medium">{company.compType}</span>
                        </div>
                        <div className="flex justify-between text-xs text-gray-400">
                          <span>업종:</span>
                          <span className="font-medium">{company.compIndustry}</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-purple-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
