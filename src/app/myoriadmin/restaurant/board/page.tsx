...
interface BoardData {
  ...
  restaurant?: { restaurantIdx: number; restaurantName: string };
}
...
const RestaurantBoardManagementPage: React.FC = () => {
  ...
  const searchBoards = async (keyword = "", page = 1, pageSize = rowsPerPage) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/restaurantboard?${params.toString()}`, {
      ...
      console.error("맛잘알오빠 후기 검색 실패:", e);
      setBoards([]);
      setTotalPages(1);
      setCurrentPage(page);
    } finally {
  ...
  const handleAdd = async () => {
    try {
      const accessToken = getToken();
      const payload = { boardTitle: newBoard.boardTitle, boardContent: newBoard.boardContent, boardID: newBoard.boardID, boardPW: newBoard.boardPW, category: '맛잘알' } as any;
      const res = await fetch(`${API_BASE_URL}/admin/restaurantboard`, {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        const res = await fetch(`${API_BASE_URL}/admin/restaurantboard/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" } });
  ...
  return (
    <div className="flex h-full gap-5">
      <div className="w-1/2 bg-white rounded-2xl border border-gray-100 p-5 flex flex-col">
        <div className="mb-5">
          <h2 className="text-lg font-bold text-gray-900">맛잘알오빠 후기 관리</h2>
                  ...
                  boards.map((b) => (
                    <tr key={b.boardIdx} className={`border-t border-gray-50 cursor-pointer transition-colors ${selectedBoard?.boardIdx === b.boardIdx ? 'bg-purple-50/70' : 'hover:bg-gray-50/70'}`}
                      onClick={async () => {
                        ...
                        try {
                          const accessToken = getToken();
                          const res = await fetch(`${API_BASE_URL}/admin/restaurantboard/${b.boardIdx}?includeDeleted=1`, { headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" } });
                      ...
                      <td className="px-3 py-2.5 text-sm text-gray-600 truncate max-w-[120px]">{b.restaurant?.restaurantName || '-'}</td>
...
export default RestaurantBoardManagementPage;
