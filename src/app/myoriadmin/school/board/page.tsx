...
const UnivBoardManagementPage: React.FC = () => {
  ...
  const searchBoards = async (keyword = "", page = 1, pageSize = rowsPerPage) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/univboard?${params.toString()}`, {
        headers: {
  ...
  const handleAdd = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/univboard`, {
        method: "POST",
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        const res = await fetch(`${API_BASE_URL}/admin/univboard/${id}`, {
          method: "DELETE",
          headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
        });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/univboard/${editingBoard.boardIdx}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
  ...
  return (
    <div className="flex h-full gap-5">
      ...
      <div className="w-1/2 bg-white rounded-2xl border border-gray-100 p-5 flex flex-col">
        {/* 헤더 */}
        <div className="mb-5">
          <h2 className="text-lg font-bold text-gray-900">학교오빠 후기 관리</h2>
                  ...
                  boards.map((b) => (
                    <tr
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const accessToken = getToken();
                          const res = await fetch(`${API_BASE_URL}/admin/univboard/${b.boardIdx}?includeDeleted=1`, {
                            headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
                          });
                          if (res.ok) {
...
export default UnivBoardManagementPage;
