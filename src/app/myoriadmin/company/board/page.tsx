...
interface BoardData {
  ...
  boardHits?: string;
  compIdx?: string;
  company?: { compIdx: number; compName: string };
}
...
const CompBoardManagementPage: React.FC = () => {
  ...
  const searchBoards = async (keyword = "", page = 1, pageSize = rowsPerPage) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/compboard?${params.toString()}`, {
        headers: {
  ...
  const handleAdd = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/compboard`, {
        method: "POST",
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        const res = await fetch(`${API_BASE_URL}/admin/compboard/${id}`, {
          method: "DELETE",
          headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
        });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      (Object.keys(editingBoard) as (keyof BoardData)[]).forEach((k) => {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/compboard/${editingBoard.boardIdx}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        setSelectedBoard({ ...selectedBoard, ...body } as BoardData);
  ...
  return (
    <div className="flex h-full gap-5">
      <div className="w-1/2 bg-white rounded-2xl border border-gray-100 p-5 flex flex-col">
        <div className="mb-5">
          <h2 className="text-lg font-bold text-gray-900">회사오빠 후기 관리</h2>
                  ...
                  boards.map((b) => (
                    <tr
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const accessToken = getToken();
                          const res = await fetch(`${API_BASE_URL}/admin/compboard/${b.boardIdx}?includeDeleted=1`, {
                            headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
                          });
                          if (res.ok) {
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-sm text-gray-600 truncate max-w-[120px]">{b.company?.compName || '-'}</td>
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => goToPage(1)} disabled={currentPage === 1} className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs transition-colors ${currentPage === 1 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100'}`}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 19l-7-7 7-7m8 14l-7-7 7-7" /></svg>
            </button>
            <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs transition-colors ${currentPage === 1 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100'}`}>
            ...
            <div className="flex items-center gap-1 px-2">
              <input
                ...
                onBlur={() => { const page = parseInt(tempPage); if (!isNaN(page)) { goToPage(Math.min(Math.max(page, 1), totalPages)); } else { setTempPage(String(currentPage)); } }}
                onKeyDown={(e) => { if (e.key === 'Enter') { const page = parseInt(tempPage); if (!isNaN(page)) { goToPage(Math.min(Math.max(page, 1), totalPages)); } else { setTempPage(String(currentPage)); } } }}
            ...
            <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs transition-colors ${currentPage === totalPages ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100'}`}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
            </button>
            <button onClick={() => goToPage(totalPages)} disabled={currentPage === totalPages} className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs transition-colors ${currentPage === totalPages ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100'}`}>
        ...
        {isAddMode ? (
          <div className="flex flex-col h-full">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">후기 추가</h3>
                <p className="text-xs text-gray-400">새 후기를 작성하세요</p>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto space-y-4 pr-1">
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5">제목</label>
                <input type="text" value={newBoard.boardTitle} onChange={(e) => setNewBoard({ ...newBoard, boardTitle: e.target.value })} className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all" placeholder="제목을 입력하세요" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5">내용</label>
                <textarea value={newBoard.boardContent} onChange={(e) => setNewBoard({ ...newBoard, boardContent: e.target.value })} className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all resize-none" rows={8} placeholder="내용을 입력하세요" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5">작성자 ID (선택)</label>
                  <input type="text" value={newBoard.boardID} onChange={(e) => setNewBoard({ ...newBoard, boardID: e.target.value })} className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5">비밀번호 (선택)</label>
                  <input type="password" value={newBoard.boardPW} onChange={(e) => setNewBoard({ ...newBoard, boardPW: e.target.value })} className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all" />
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className={`px-2 py-0.5 text-xs font-medium rounded-md ${selectedBoard.isDeleted === 1 ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>
                ...
                {!isEditMode && (
                  <div className="flex gap-1.5">
                    <button onClick={handleEditStart} className="w-8 h-8 rounded-lg bg-white shadow-sm border border-gray-200 hover:border-purple-300 hover:bg-purple-50 flex items-center justify-center transition-all" title="수정">
                    ...
                    <button onClick={async () => { setSelectedIds([selectedBoard.boardIdx]); await handleDelete(); setSelectedBoard(null); }} className="w-8 h-8 rounded-lg bg-white shadow-sm border border-gray-200 hover:border-rose-300 hover:bg-rose-50 flex items-center justify-center transition-all" title="삭제">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="w-full px-3.5 py-2.5 text-sm text-gray-500 bg-gray-50/80 border border-gray-100 rounded-xl">
                      {selectedBoard.boardRegDate ? new Date(selectedBoard.boardRegDate).toLocaleString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '-'}
                    </div>
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1.5">삭제 여부</label>
                  <div className={`w-full px-3.5 py-2.5 text-sm border border-gray-100 rounded-xl ${selectedBoard.isDeleted === 1 ? 'bg-rose-50/80 text-rose-700' : 'bg-emerald-50/80 text-emerald-700'}`}>
...
export default CompBoardManagementPage;
